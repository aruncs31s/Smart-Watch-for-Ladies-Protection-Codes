...
Numbers number = {"8593823071", "9539942563"}; // Police and Parent
...
void sendText(const char *number) {
...
void getLocation() {
  ...
  Serial.println(location._lat);
  Serial.println(location._long);
  location._gps_status ^= 1;
}
void sendSOS() {
  /*
   ...
   */
  getLocation();
  sendText(number.parent);
  sendText(number.police);
}
